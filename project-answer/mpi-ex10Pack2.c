...
int main(int argc, char *argv[])
{
  ...
  if(rank==0){
    ...
    while(fscanf(fp,"%s%s",st[nc].stID,st[nc].name)!=EOF){
      for(i=0;i<3;i++) fscanf(fp,"%d",&st[nc].point[i]);
      nc++;
    }  // rank0がstudentData.txtからすべてのdataを読む
    fclose(fp);

    NE=nc/size;  NR=nc%size;
    // printf("全学生数：%d名, NE=%d, NR=%d\n", nc,NE,NR); fflush(stdout);
    for(i=1; i<size; i++){
      starti=NE*i+((NR>i)?i:NR); ndata=NE+((NR>i)?1:0);
      position=0;
    ...
    for(i=0;i<nc0;i++){
      ...
      if(max<st[i].point[3]) max=st[i].point[3];
    }
    ...
    printf("Each max-->");
    for(i=0; i<size; i++) printf("%4d",maxp[i]);
    printf("\n");
    // 真の最大値を他のrankに知らせる
    for(i=1; i<size; i++)
    ...
    // 自分の担当学生の中の最大得点者を探す
    for(i=0;i<nc0;i++)
      if(st[i].point[3]==max) top[nmax++]=st[i];
